...
const Navbar = () => {
  ...
  const socialLinks = [
    ...
    {
      name: "Email",
      url: "mailto:sandeeprajput88066@gmail.com",
      icon: FiMail,
    },
    {
      name: "X",
      url: "https://x.com/Sandeep__io",
      icon: SiX,
    },
  ];
      ...
      >
        <div className="py-2">
          ...
          <div className="flex items-center justify-between h-16 md:h-20 px-4">
            ...
            <Link
              to="/"
              className="font-bebas-neue text-4xl sm:text-5xl lg:text-6xl font-bold"
              aria-label="Sandeep - Home"
            >
              <FuzzyText
                fontSize="3rem"
                fontWeight={900}
            ...
            <div className="flex items-center gap-4">
              ...
              {/* Menu Button */}
              <button
                className="flex items-center text-midcolor border border-midcolor/50 px-4 py-2 rounded-full text-base font-semibold transition-all duration-300 hover:bg-midcolor hover:text-background hover:border-midcolor"
                onClick={() => setMenuOpen(!menuOpen)}
                aria-label="Toggle menu"
                aria-expanded={menuOpen}
              >
        ...
        <ul className="flex flex-col items-center justify-center space-y-1 w-full">
          {navLinks.map((link, index) => (
            <li key={link.name} className="w-full text-center border-b border-midcolor/20">
              <Link
                to={link.path}
                className={`inline-block w-full py-6 text-3xl sm:text-4xl lg:text-5xl font-extrabold transition-all duration-300  uppercase ${isActiveLink(link.path)
